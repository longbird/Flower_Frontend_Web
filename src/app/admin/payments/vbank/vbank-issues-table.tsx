...
import type { AdminVbankIssueRow, VbankIssueStatus } from '@/lib/payments/vbank-payment-types';
...
const STATUS_FILTERS: Array<{ label: string; value: VbankIssueStatus | 'ALL' }> = [
  { label: '전체', value: 'ALL' },
  { label: '열림', value: 'OPEN' },
  { label: '확인됨', value: 'ACKED' },
  { label: '해결됨', value: 'RESOLVED' },
  { label: '억제됨', value: 'SUPPRESSED' },
];
...
export function VbankIssuesTable() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => listVbankIssues({ status: status === 'ALL' ? undefined : status, page: 1, pageSize: 20 }),
  });
