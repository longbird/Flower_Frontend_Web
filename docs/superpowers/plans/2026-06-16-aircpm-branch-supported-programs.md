...
**기존 테스트 2개를 반드시 아래로 교체**(append 하지 말 것 — 그대로 두면 params 길이 5 vs 9 불일치로 FAIL):

```ts
// (교체) 기존 'upsert → cardPaymentEnabled 생략 시 기존 값 보존(COALESCE)'
it('upsert → cardPaymentEnabled 생략 시 기존 값 보존(COALESCE)', async () => {
  const { pool, calls } = makePool();
  const db = new AircpmBranchDb(pool as any);
  await db.upsert({ brchCd: 'S001', name: '강남' });
  expect(calls[0].sql).toMatch(/card_payment_enabled = COALESCE\(\?, card_payment_enabled\)/);
  expect(calls[0].params).toEqual(['S001', '강남', null, null, null, '강남', null, null, null]);
});

// (교체) 기존 'upsert → name 생략 시 기존 값 보존(COALESCE)'
it('upsert → name 생략 시 기존 값 보존(COALESCE)', async () => {
  const { pool, calls } = makePool();
  const db = new AircpmBranchDb(pool as any);
  await db.upsert({ brchCd: 'S001', cardPaymentEnabled: false });
  expect(calls[0].sql).toMatch(/name = COALESCE\(\?, name\)/);
  expect(calls[0].params).toEqual(['S001', null, 0, null, null, null, 0, null, null]);
});
```

기존 `'findByCode → 행 매핑'`(copy/paste 없는 버전)과 `'upsert → 신규 지사...'` 테스트는 위 신규 블록으로 **교체**(중복 방지).
...
**Files:**
- Modify: `apps/api/src/aircpm/payment/dto/aircpm-branch.dto.ts` (`UpsertBranchDto`)
- Modify: `apps/api/src/aircpm/payment/aircpm_branch.service.ts` (`upsertBranch` 시그니처)
- Test: `apps/api/src/aircpm/payment/aircpm_branch.service.spec.ts`
- Test(신규): `apps/api/src/aircpm/payment/dto/aircpm-branch.dto.spec.ts`
...
- [ ] **Step 5: DTO 검증 테스트 작성** (신규 `apps/api/src/aircpm/payment/dto/aircpm-branch.dto.spec.ts`) — 스펙 §6 "길이 5 아닌 입력 거부"

```ts
import 'reflect-metadata';
import { plainToInstance } from 'class-transformer';
import { validateSync } from 'class-validator';
import { UpsertBranchDto } from './aircpm-branch.dto';

function errs(payload: any) {
  return validateSync(plainToInstance(UpsertBranchDto, payload), { whitelist: true });
}

describe('UpsertBranchDto.copyApps/pasteApps 검증', () => {
  it('길이 5 불리언 배열 → 통과', () => {
    expect(errs({ brchCd: 'S001', copyApps: [true, false, true, false, true] })).toHaveLength(0);
  });
  it('copyApps 생략 → 통과(옵셔널)', () => {
    expect(errs({ brchCd: 'S001' })).toHaveLength(0);
  });
  it('길이 4 → 거부', () => {
    expect(errs({ brchCd: 'S001', copyApps: [true, true, true, true] }).length).toBeGreaterThan(0);
  });
  it('불리언 아님 → 거부', () => {
    expect(errs({ brchCd: 'S001', pasteApps: [1, 0, 1, 0, 1] }).length).toBeGreaterThan(0);
  });
});
```
...
Run: `cd apps/api && npx jest src/aircpm/payment/aircpm_branch.service.spec.ts src/aircpm/payment/dto/aircpm-branch.dto.spec.ts`
Expected: PASS

- [ ] **Step 7: 커밋**

```bash
git add apps/api/src/aircpm/payment/dto/aircpm-branch.dto.ts apps/api/src/aircpm/payment/dto/aircpm-branch.dto.spec.ts apps/api/src/aircpm/payment/aircpm_branch.service.ts apps/api/src/aircpm/payment/aircpm_branch.service.spec.ts
git commit -m "feat(aircpm): 지사 upsert DTO/서비스에 copy/paste apps(5슬롯) 추가 + DTO 검증"
```
...
> **유지(삭제 금지):** 기존 `로그인 settings.cardPayEnabled — 지사 카드결제 ON` 테스트의 `h.calls[4].sql` 단언들(`/LEFT JOIN aircpm_branch/`, `/u\.is_active\s*=\s*1/`)과 비활성 사용자 Red-Green 조건부 mock 테스트는 그대로 둔다. getSettings 지사 쿼리는 여전히 `WHERE u.user_id=? AND u.is_active=1` 를 유지하므로 통과한다.
...
### Task 7: 지사 API 클라이언트 타입

> **주의(커밋 그린 유지):** `src/lib/api/aircpm.ts` 의 `AircpmUserSettings`/`AircpmUserSettingsPatch` 변경은 사용자 설정 페이지 편집과 한 커밋에 묶어야 TS가 깨지지 않으므로 **Task 9 로 이동**. 본 Task는 `aircpm-payments.ts`(지사 타입)만 수정한다. 지사 타입에 필드를 추가해도 지사 페이지(Task 8 이전)는 해당 필드를 읽지 않으므로 타입 깨짐 없음.

**Files:**
- Modify: `src/lib/api/aircpm-payments.ts` (`AircpmBranch` 타입 + `upsertAircpmBranch` 바디)
...
> `aircpm.ts`(`AircpmUserSettings`/`AircpmUserSettingsPatch`) 변경은 Task 9에서 함께 수행.
...
git add src/lib/api/aircpm-payments.ts src/__tests__/aircpm/aircpm-payments-api.test.ts
git commit -m "feat(aircpm): 지사 API 타입에 copy/paste apps 추가"
```
...
### Task 9: 사용자 설정 페이지 — copy/paste 제거 (+ `aircpm.ts` 타입)

**Files:**
- Modify: `src/app/aircpm/users/[userId]/settings/page.tsx`
- Modify: `src/lib/api/aircpm.ts` (`AircpmUserSettings`/`AircpmUserSettingsPatch`) — **페이지 편집과 동일 커밋**(TS 그린 유지)
...
- [ ] **Step 3: 구현** — `settings/page.tsx` (라인번호 대신 내용 기준 식별)

- 최상단 `const APP_LABELS ...` 상수 선언 제거.
- `const [copyApps, setCopyApps] = useState(...)` 와 `const [pasteApps, setPasteApps] = useState(...)` state 선언 제거.
- `toggleCopy` / `togglePaste` 핸들러 함수 제거.
- `useEffect` 초기화 블록 안의 `setCopyApps(data.copyApps)` / `setPasteApps(data.pasteApps)` 두 줄 제거 (나머지 setAppTitle/setPriceUp/setTelegram* 은 유지).
- `saveMutation` 의 `body` 객체에서 `copyApps,` / `pasteApps,` 두 줄 제거.
- "복사 대상 앱 (copyApps)" 와 "붙여넣기 대상 앱 (pasteApps)" 두 `<Card>` 블록 전체 제거.
- 헤더 설명 `<p>` 문구를 `"앱 타이틀, 단가 인상, 진단 로그 Telegram 자격을 편집합니다."` 로 수정.

- [ ] **Step 4: `aircpm.ts` 타입 수정** (`src/lib/api/aircpm.ts`)
...
Expected: PASS
Run: `npx tsc --noEmit`
Expected: 0 errors (페이지·타입 동일 커밋으로 4-튜플 ← `boolean[]` 충돌 없음)

- [ ] **Step 6: 커밋**

```bash
git add "src/app/aircpm/users/[userId]/settings/page.tsx" src/lib/api/aircpm.ts src/__tests__/aircpm/user-settings-page.test.tsx
