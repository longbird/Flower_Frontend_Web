...
- `/o/[code]` 페이지를 10블록 카드 구조로 재구성하고, 목업 디자인의 정보 항목을 모두 흡수한다.
...
2. 주문 정보 10블록(상품/배송/받는분/리본/증빙/인수자 등)을 확인
...
| 8 | 인수자 정보 | order.status === DELIVERED 일 때만 (FE 필터링) | deliveryPhotos, scenePhotos, recipientActualName, receivedAt, recipientRelationship |
| 9 | 액션 영역 (수정 요청 + 주문 확인 완료) | 항상 (수정 요청은 항상, 확인 완료는 동적) | branchPhone, customerConfirmedAt |
| 10 | Footer (지사 전화) | 항상 | branchName, branchPhone |

**액션 영역 동작 (블록 9)**
- **수정 요청 버튼**: `<a href={`tel:${branchPhone}`}>` outline 버튼. **고객 확인 완료 여부와 무관하게 항상 표시**. branchPhone 없으면 비활성화 + "지사 연락처 미등록" 안내.
- **주문 확인 완료 버튼**:
  - `customerConfirmedAt == null` → 활성 버튼. 클릭 → confirm 다이얼로그("주문 내용에 동의하고 확인 완료하시겠습니까?") → `POST /public/order-link/{code}/confirm` → 성공 시 응답의 `customerConfirmedAt`로 즉시 텍스트 교체.
  - `customerConfirmedAt != null` → 처음부터 "✓ 확인 완료됨 (yyyy-mm-dd hh:mm)" 텍스트만 표시 (버튼 없음).
  - 두 경우 모두 수정 요청 버튼은 함께 표시.

**인수자 정보 섹션 (블록 8)**
- BE는 `scenePhotos`/`deliveryPhotos`를 status와 무관하게 항상 배열로 반환 (빈 배열 가능).
- FE는 `order.status === 'DELIVERED' || 'ORDER_DELIVERED'`일 때만 카드를 렌더 (status 게이팅).
- 카드 내부: 배송사진/현장사진 두 그리드 (각 2열). 각 그룹 라벨 명시.
- 그룹별로 사진 없음이면 해당 그룹만 숨김. 둘 다 없고 인수자 3필드도 모두 null이면 카드 전체 숨김.
- 인수자명/인수시간/관계는 InfoRow로 표시. null이면 "-".

**branchPhone 출처**
- `branchPhone`은 현재 `CustomerOrderView`에 이미 존재함 (`src/lib/api/order-link.ts`에 `branchPhone?: string | null` 명시). Phase 1 BE 작업에 새로 추가할 필요 없음.
...
- 카드 마운트 시 인수자 3필드는 **기존 admin order detail GET 응답** (`GET /admin/orders/{id}`)에서 읽어옴 — 별도 GET 추가 없음. Phase 1 BE 작업: 이 응답에 `recipientActualName`, `receivedAt`, `recipientRelationship` 필드를 함께 노출하도록 확장.
- 사진 목록은 `GET /admin/orders/{id}/proofs` 신규 엔드포인트로 별도 조회.
- 탭별 그리드가 비어있을 때: "등록된 사진이 없습니다" 안내 + 추가 버튼은 그대로 노출.
...
- 인수자 3필드는 partner order detail GET (`GET /partner/orders/{id}`) 응답에서 초기값 로드 — Phase 1 BE에서 이 응답도 같이 확장 필요.
- 기존 partner의 `presignProof` / `completeProof` 호출 시그니처는 변경 없음. `proofType` 파라미터에 신규로 `'SCENE_PHOTO'`를 보낼 수 있도록 BE만 수용 (FE는 이미 string으로 보내므로 코드 변경 불필요).
- 탭별 그리드 비어있을 때: 기존 "등록된 증빙 사진이 없습니다" 안내 그대로 사용.
...
`customer_confirmation_ip`는 v1에서 노출/저장하지 않음. 추후 분쟁 로깅 필요시 별도 작업으로 추가.

### 6.2 proofs 테이블
- BE 작업 첫 단계에서 현재 `proof_type` 컬럼 정의(enum / varchar)를 확인하고 spec에 결과를 기록한다.
- enum이면 `SCENE_PHOTO` 추가. varchar이면 코드만 추가하면 됨.
- 기존 `DELIVERY_PHOTO`는 그대로 유지.
- **Phase 1 acceptance criterion**: `SCENE_PHOTO`로 row insert가 성공함을 BE 단위 테스트로 확인.
...
| POST | `/public/order-link/{token}/confirm` | `{}` | `{ok:true, customerConfirmedAt: string}` | public + token |

**기존 partner 사진 엔드포인트** (`/partner/orders/{id}/proofs/presign`, `complete`, `proofs`)는 시그니처 그대로 유지. BE는 `proofType` 필드로 `'SCENE_PHOTO'`를 받아 처리할 수 있도록 검증/저장 로직만 확장.

### 7.2 기존 엔드포인트 확장

**`GET /public/order-link/{token}` 응답** — 다음 필드 추가:
```ts
{
  // ... 기존 필드 (branchName, branchPhone 등은 이미 존재함)
  order: {
    // ... 기존
  ...
  deliveryPhotos: DeliveryPhoto[];   // proofType=DELIVERY_PHOTO, 항상 배열 (빈 배열 가능)
  scenePhotos: DeliveryPhoto[];      // proofType=SCENE_PHOTO ← 신규, 항상 배열
}
```

**`GET /admin/orders/{id}` 응답** — 다음 필드 추가:
- `recipientActualName`, `receivedAt`, `recipientRelationship`

**`GET /partner/orders/{id}` 응답** — 다음 필드 추가:
- `recipientActualName`, `receivedAt`, `recipientRelationship`

### 7.3 멱등성
- `POST /public/order-link/{token}/confirm`은 idempotent.
  - 첫 호출: `customer_confirmed_at = NOW()` 기록 → `{ ok: true, customerConfirmedAt: <now> }` 반환.
  - 재호출: DB는 변경 없음. 응답 shape은 동일하되 `customerConfirmedAt`은 **기존 저장값**을 그대로 반환.
...
src/__tests__/customer-order-view.test.tsx
  ...
  - 비-DELIVERED 상태에서 인수자 카드 미렌더 (사진 배열이 있어도)
  - 미확인 상태에서 [주문 확인 완료] 버튼 렌더, confirm POST 후 텍스트로 전환
  - 이미 confirmed (customerConfirmedAt != null) 상태에서 처음부터 텍스트만 표시
  - 수정 요청 버튼은 confirmed 여부와 무관하게 항상 표시
  - 수정 요청 버튼 href=tel:{phone}
  - branchPhone null + confirmed 조합: 수정 요청 비활성 + 확인 완료 텍스트 동시 표시
  - 섹션 표시 조건 검증 (10블록 매트릭스)

src/__tests__/admin-order-delivery-card.test.tsx
  - 탭 전환 시 proofType 분리
  - 인수자 폼 초기값이 admin order detail GET에서 로드됨
  - 인수자 폼 저장 호출 검증
  - 사진 0개일 때 "등록된 사진이 없습니다" 안내

src/__tests__/partner-order-recipient.test.tsx
  - 인수자 정보 저장
  - 배송완료 시 미입력 confirm
  - 사진 탭 전환 시 proofType 'SCENE_PHOTO'로 업로드 호출
```
...
- 마이그레이션 SQL 실행 후 4개 컬럼이 존재함 (`SHOW COLUMNS FROM orders LIKE '%recipient%'` 등으로 확인)
- `proofs.proof_type`에 `'SCENE_PHOTO'`로 row insert 가능
- 다음 curl이 신규 필드를 모두 포함한 JSON을 반환:
  ```
  curl -s http://test-server/api/proxy/public/order-link/{token} | jq .
  ```
  포함되어야 할 신규 키 11개: `productName`, `senderName`, `invoiceMethod`, `funeralHall`, `hallName`, `roomNumber`, `recipientActualName`, `receivedAt`, `recipientRelationship`, `customerConfirmedAt`, `scenePhotos`
- `GET /admin/orders/{id}` 와 `GET /partner/orders/{id}` 응답에도 인수자 3필드 포함 검증
- 6개 신규 엔드포인트 각각 200 응답 확인 (`/admin/orders/{id}/proofs/presign`, `complete`, `proofs`, `recipient-info`, `/partner/orders/{id}/recipient-info`, `/public/order-link/{token}/confirm`)
