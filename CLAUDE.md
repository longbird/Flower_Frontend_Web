...
### 배포 방법 (제로 다운타임 — 2026-03-31~)

symlink 기반 Blue-Green 배포 + PM2 cluster graceful reload. 다운타임 0초.
...
```
/home/blueadm/
  frontend_web          ← symlink → releases/현재버전/
  releases/
    20260331_HHMMSS/    ← 릴리스들 (최근 3개 유지)
  shared/
    .env.local          ← 모든 릴리스가 symlink으로 참조
```

- PM2: cluster 모드, 2 인스턴스 (`ecosystem.config.js`)
- 배포 흐름: 새 릴리스 빌드 → symlink 전환 (atomic) → pm2 reload → 헬스체크 → 실패 시 자동 롤백

### 절대 금지 사항

- **`shared/.env.local`을 삭제하지 마라** — 모든 릴리스가 참조하는 공유 파일. 삭제하면 전체 API 장애
- **수동으로 `frontend_web/` 내용을 직접 수정하지 마라** — symlink이므로 항상 `deploy-zerodt.sh`로 배포
- **`pm2 restart` 대신 `pm2 reload`를 사용하라** — restart는 다운타임 발생, reload는 graceful
...
### 배포 후 검증 (deploy-zerodt.sh가 자동 수행)
